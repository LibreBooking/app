...
class SmartyPage extends Smarty
{
	...
	public function DeleteButton($params, &$smarty)
	{
		...
		$class = isset($params['class']) ? $params['class'] : '';
		echo '<button type="button" class="btn btn-danger save ' . $class . '" ' . $this->GetButtonAttributes($params) . '><span class="glyphicon glyphicon-trash"></span> ' . Resources::GetInstance()
																																														->GetString($key) . '</button>';
	}
