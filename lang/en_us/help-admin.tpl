	...
	<div class="row">
		...
		<div id="help" class="col s12 m9">
			...
			<p>Credits allow control over a user's usage. Credits must first be enabled in the application configuration
				before they can be managed. The setting is $conf['settings']['credits']['enabled'] Once enabled, administrators will have the ability to set the
				credit redemption
			...
			<p>To set or adjust the credit balance for a user, an administrator can open the user management tool and adjust the current number of credits for
				each usage.
				Alternatively, an administrator can open the group management tool and add a specific amount of credits to all users in a group.</p>

			<h3 id="help-payments">Paying for Reservation Usage</h3>

			<p>To charge for reservation usage, users can purchase credits. This can be enabled by setting $conf['settings']['credits']['allow.purchase'] to
				true.
				From the Payments section of Application Management administrators are able to set the cost per credit, configure payment gateways, and view
				purchase transaction history.</p>
			...
			<h3>Automatically Replenishing Credits</h3>
			<p>Automatic credit replenishment is configured in the group management tool. To automatically add credits to a group of users on a schedule, click
				the Replenish link
				for that group and set the replenishment rules. It is possible to add a set amount of credits every certain number of days, or on a specific day
				of each month.</p>

			<p>It is important to remember that a user will receive credits for each group that they are part of.
				So if a user belongs to group 1 and group 2 and both groups are set to replenish 5 credits per day, that user will receive 10 credits per
				day.</p>

			<p>The replenishment scheduled job must be configured for automatic credit replenishment to function. See the <a href="#jobs">jobs section</a> of
				the
				help page to see the details on configuring the replenishment job.</p>
			...
			<p>On Linux, a cron job can be used. The command to run is <span class="note">php</span> followed by the
				full
				path to
				Jobs/sendwaitlist.php. The full path on this server is <span
			...
			<h3 id="help-replenishment">Automatic Credit Replenishment</h3>

			<p>Administrators can automatic credit replenishment rules for groups of users. For this
				feature to function, $conf['settings']['credits']['enabled'] must be set to true. Also, a scheduled task must be configured on your server to execute
				Jobs/replenishcredits.php once per day</p>

			<p>On Linux, a cron job can be used. The command to run is <span class="note">php</span> followed by the
				full
				path to
				Jobs/replenishcredits.php. The full path this server is <span
						class="note">{$ReplenishmentPath}</span>
			</p>

			<p>An example cron configuration might look like: <span
						class="note">0 0 * * * php -f {$ReplenishmentPath}</span>
			</p>
			...
			<p>If you have access to cPanel through a hosting provider, <a
						...
						target="_blank">setting
					up
					a cron job in cPanel</a> is straightforward. Either select the Every Day option from the
				Common
				Settings menu,
				or enter 0 for minute and hour, and * for day, month and weekday.</p>
			...
			<p>On Windows, <a href="http://windows.microsoft.com/en-au/windows7/schedule-a-task" target="_blank">a
				...
				The task to execute is php followed by
				the
				full path to {$AppTitle}\Jobs\replenishcredits.php. For example, c:\PHP\php.exe -f
				c:\inetpub\wwwroot\Booked\Jobs\replenishcredits.php</p>
			...
			<p>Available tokens for reservation labels
				...
				{ldelim}name{rdelim}, {ldelim}title{rdelim}, {ldelim}description{rdelim}, {ldelim}email{rdelim}, {ldelim}phone{rdelim}, {ldelim}
				organization{rdelim}, {ldelim}position{rdelim}, {ldelim}startdate{rdelim}, {ldelim}enddate{rdelim} {ldelim}resourcename{rdelim} {ldelim}
				participants{rdelim} {ldelim}invitees{rdelim} {ldelim}reservationAttributes{rdelim}
			...
			<p>More info for CAS configuration can be found at <a
						href="https://wiki.jasig.org/display/casc/phpcas">https://wiki.jasig.org/display/casc/phpcas</a>
			</p>
			...
			<div>Additional support options, including paid enterprise support, can be found at <a href="https://www.twinkletoessoftware.com/services" target="_blank">https://www.twinkletoessoftware.com/services</a>.</div>
...
<script type="text/javascript">
	$(function () {
		tocbot.init({
			tocSelector: '#toc-div', contentSelector: '#help', headingSelector: 'h2, h3', orderedList: false
		});
