...
<p>Chaque planning doit avoir une grille de créneaux horaires définie. Cela permet de controler la disponibilité des ressources.
...
<p>Un créneau avec texte sera de la forme:: "10:25 - 16:50 Texte de Mon Creneau"</p>

<p>Sous la liste des créneaux un assistant vous permettra de crééer ceux-ci à des intervalles fixes.</p>

<h3>Mettre en place des Resources</h3>
...
<p>Demander une approbation pour une ressource entrainera une "mise en attente" des réservations jusqu'a approbation par le responsable (par défaut pas d'approbation)</p>
...
Par exemple, Nous sommes un Lundi à 10h30 et la ressource ne peut être réservée à plus d'un jour dans le futur. Ainsi celle-ci ne sera réservable que jusqu'à Mardi à 10h30 (par defaut pas de maximum).</p>
...
Fixer cette capacité (organisateur non inclus) permet d'adapter le nombre de participants (raisosns de sécurité par exemple) aux possibilité de cette salle(par défaut les ressources ont une capacité illimitée).</p>
...
cela nécéssite l'installation de php_gd2 et son activation dans le fichier php.ini de votre serveur.
 <a href="http://www.php.net/manual/en/book.image.php">Plus de détails...</a></p>

<h3>Gérer les Accessoires</h3>

<p>On envisage les accessoires comme des objets utilisés lors de reservations. par exemple: des vidéoprojecteurs dans une salle.</p>

<p>Les Accessoires peuvent être gérés via le sous-menu "Accessoires" du sous-menu "Ressources". Vous pouvez limiter le nombre d'accessoires réservés en fixant une quantité maximum par reservation.</p>
...
<p>Les groupes permettent d'organiser les utilisateurs, de controler les permissions d'accès aux ressources et de définir des rôles dans l'application.</p>

<h3>Rôles</h3>

<p>Les rôles donnent à un groupe l'autorisation d'effectuer certaines actions</p>

<p>Les utilisateurs qui appartiennent à un groupe qui a le rôle d'Administrateur d'Application ont tous les droits d'aministration.
Ce rôle n'a presque aucune restriction quant aux reservations de ressources. Il peut gérer presque toute l'application.</p>

<p>Les utilisateurs qui appartiennent à un groupe qui a le rôle d'Administrateur de rôle ont le droit de réserver pour quelqu'un d'autre ainsi que gérer le utilisateurs de ce groupe.</p>

<h3>Gérer les Réservations</h3>

<p>via le sous-menu "Réservations". Par defaut, les 7 jours passés et suivants la date en cours sont affichés.
...
<h3>Approbation de Réservation</h3>

<p>Depuis la liste des réservation vous pourrez effectuer les approbations de réservations. Celles qui sont en attente sont surlignées.</p>

<h3>Gérer les Utilisateurs</h3>

<p>Via le sous-menu "Utilisateurs". Il y est possible de changer les droits d'acces aux ressources d'un utilisateur, d'ajouter, de modifier, désactiver, ou effacer des comptes, de réinitialiser des mots de passe.</p>
...
<p class="setting"><span>$conf['settings']['allow.self.registration']</span>Determine si les utilisateurs peuvent eux même créer de nouveaux comptes.</p>
...
<p class="setting"><span>$conf['settings']['image.upload.url']</span>L'URL relative au script.url où les images uploadés peuvent être vus</p>

<p class="setting"><span>$conf['settings']['cache.templates']</span>Met en cache (ou non) les templates. Il est recommandé de le rêgler à "true", tant que "tpl_c" est modifiale (droits en ecriture)</p>

<p class="setting"><span>$conf['settings']['use.local.jquery']</span>Autorise (ou non) une version locale de jQuery.
	si fixé à "false", les fichiers seront ceux de Google CDN. Il est recommandé de mettre à
	"false" pour optimiser les performances. "false" par défaut.</p>
...
<p class="setting"><span>$conf['settings']['registration.require.email.activation']</span>Determine si les utilisateurs doivent activer leur compte par email avant de pouvoir se connecter</p>

<p class="setting"><span>$conf['settings']['registration.auto.subscribe.email']</span>Fixe l'inscription automatique aux email des utilisateurs àprès enregistrement du compte.</p>
...
<p class="setting"><span>$conf['settings']['ics']['require.login']</span>Les utilisateurs doivent-ils être connecté pour ajouter une réservation à Outlook.</p>
...
<p class="setting"><span>$conf['settings']['privacy']['hide.user.details']</span>Autorise (ou non) les non-Administrateur à voir les informations personnelles des utilisateurs. Par défaut "false".</p>

<p class="setting"><span>$conf['settings']['reservation']['start.time.constraint']</span>Determine quand les réservation peuvent être créées, modifiées. valeurs possibles : "future", "current", "none". "future" signifie que les réservations ne peuvent être créées ou modifiées si l'heure de début du créneau horaire choisi est passée."current" signifie que les réservations peuvent être créées ou modifiées si l'heure de fin du créneau horaire choisi n'est pas dépassé."none" signifie qu'il n'y a aucune restriction. Par défaut "future".</p>

<p class="setting"><span>$conf['settings']['reservation']['updates.require.approval']</span>Fixe si des réservations anciennements approuvées, puis modifiées nécéssitent une seconde approbation. Par défaut "false".</p>

<p class="setting"><span>$conf['settings']['reservation']['prevent.participation']</span>Interdit (ou non) les utilisateur d'ajouter ou d'inviter d'autres utilisateurs à une réservation.Par défaut "false".</p>

<p class="setting"><span>$conf['settings']['reservation']['prevent.recurrence']</span>Interdit au utilisateurs de créer des réservation réccurentes.Par défaut "false".</p>
...
<p class="setting"><span>$conf['settings']['database']['hostspec']</span>URL d'hebergement de la base ou "named pipe"</p>
...
<p class="setting"><span>$conf['settings']['phpmailer']['smtp.secure']</span>Sécurisation SMTP, si vous utilisez smtp. Options :
	'', "ssl" ou "tls"</p>

<p class="setting"><span>$conf['settings']['phpmailer']['smtp.auth']</span>SMTP nécésssite une authentication, si vous utilisez smtp.
	Options : "true"
	ou "false"</p>
...
<p class="setting"><span>$conf['settings']['plugins']['PreReservation']</span>Nom du répertoire du plugin gérant les preréservations. Plus d'infos sur les plugins dans le paragraphe "Plugins" ci-dessous.</p>
...
<p>
	Pour activer un plugin, remplissez la valeur du fichier de configuration avec le nom du répertoire du plugin.
	Par exemple, pour activer  "l'authentication" de "Ldap" saisissez  $conf['settings']['plugins']['Authentication'] = 'Ldap';
	</p>

<p>Les Plugins peuvent avoir leurs propre fichiers de configuration. Pour "Ldap", renommez ou copiez le fichier "/plugins/Authentication/Ldap/Ldap.config.dist" en "/plugins/Authentication/Ldap/Ldap.config" et modifiez les valeurs de ce fichier pour les adapter à votre environnement.</p>
