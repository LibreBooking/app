							...
							<div class="col s3 m9">
								...
								<div>
									...
									<a class="update changeStatus" href="#">
										<span class="no-show">{translate key=Status}</span>
										<span class="far fa-edit"></span>
									</a>
                                    {if $resource->IsAvailable()}
										<span class="resource-available fas fa-power-off"></span>
										<span>{translate key='Available'}</span>
                                    {elseif $resource->IsUnavailable()}
										<span class="resource-unavailable fas fa-power-off"></span>
										<span>{translate key='Unavailable'}</span>
                                    {else}
										<span class="resource-hidden fas fa-power-off"></span>
										<span>{translate key='Hidden'}</span>
                                    {/if}
								...
								<div>
									...
									<a class="update changeScheduleButton" href="#">
										<span class="no-show">{translate key=Schedule}</span>
										<span class="far fa-edit"></span>
									</a>
		...
		<form id="statusForm" class="statusForm" method="post"
			...
			<div class="modal-content">
				...
				<div class="input-field existingStatusReason">
					<label for="addStatusReasonId" class="active">
		...
		<form id="bulkUpdateForm" method="post" ajaxAction="{ManageResourcesActions::ActionBulkUpdate}"
			...
			<div class="modal-content">
				...
				<section>
					...
					<div>
						<label for="bulkEditNoStartNoticeAdd">
					...
					<div>
						<label for="bulkEditNoStartNoticeDelete">
					...
					<div class="input-field">
						<label for="bulkEditAllowMultiday"
							   class="control-label active">{translate key=ResourceAllowMultiDay}</label>
						<select id="bulkEditAllowMultiday" {formname key=ALLOW_MULTIDAY}>
					...
					<div class="input-field">
						<label for="bulkEditRequiresApproval"
							   class="control-label active">{translate key='ResourceRequiresApproval'}</label>
						<select id="bulkEditRequiresApproval"
								class="input-sm" {formname key=REQUIRES_APPROVAL}>
                            {html_options options=$YesNoUnchangedOptions}
						</select>
					</div>
					...
					<div class="input-field">
						<label for="bulkEditAutoAssign"
							   class="control-label active">{translate key='ResourcePermissionAutoGranted'}</label>
						<select id="bulkEditAutoAssign" class="" {formname key=AUTO_ASSIGN}>
                            {html_options options=$YesNoUnchangedOptions}
						</select>
					</div>
					...
					<div class="input-field">
						<label for="bulkEditEnableCheckIn"
							   class="control-label active">{translate key=RequiresCheckInNotification}</label>
						<select id="bulkEditEnableCheckIn" class="" {formname key=ENABLE_CHECK_IN}>
                            {html_options options=$YesNoUnchangedOptions}
						</select>
					...
					<div class="input-field">
						<label for="bulkEditAllowSubscriptions"
							   class="control-label active">{translate key='TurnOnSubscription'}</label>
						<select id="bulkEditAllowSubscriptions"
								class="" {formname key=ALLOW_CALENDAR_SUBSCRIPTIONS}>
                            {html_options options=$YesNoUnchangedOptions}
						</select>
					</div>
	...
	<div id="importDialog" class="modal modal-fixed-header modal-fixed-footer" tabindex="-1" role="dialog"
		...
		<form id="importForm" class="form" role="form" method="post" enctype="multipart/form-data"
			...
			<div class="modal-footer">
				...
				<div class="right">
                    {cancel_button}
                    {add_button key=Import submit=true}
                    {indicator}
				</div>
				<div class="clearfix"></div>
			</div>
