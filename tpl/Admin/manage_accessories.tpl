...
<div id="page-manage-accessories" class="admin-page">
				...
				<div class="col-xs-5">
					<div class="form-group has-feedback">
						...
						<i class="glyphicon glyphicon-asterisk form-control-feedback"
						   data-bv-icon-for="accessoryName"></i>
					</div>
				</div>
				<div class="col-xs-7">
			...
			<form id="editForm" method="post">
				<div class="modal-content">
					...
					<div class="modal-body">
                        ...
                        {if $CreditsEnabled}
							<div class="form-group col-xs-4">
								<label for="editCredits">{translate key='CreditsOffPeak'}</label>
								<input id="editCredits" type="number" min="0" class="form-control" {formname key=CREDITS} />
							</div>
							<div class="form-group col-xs-4">
								<label for="editPeakCredits">{translate key='CreditsPeak'}</label>
							...
							<div resource-id="{$resourceId}">
								...
								<div class="quantities no-show form-group-sm">
									<label>{translate key=MinimumQuantity}
										<input type="number" min="0" data-type="min-quantity" class="form-control"
											   size="4" maxlength="4"
											   name="{FormKeys::ACCESSORY_MIN_QUANTITY}[{$resource->GetId()}]"></label>
									<label>{translate key=MaximumQuantity}
										<input type="number" min="0" data-type="max-quantity" class="form-control"
											   size="4" maxlength="4"
	...
	<script type="text/javascript">
		$(document).ready(function () {
            ...
            {foreach from=$accessories item=accessory}
			accessoryManagement.addAccessory('{$accessory->Id}', '{$accessory->Name|escape:javascript}', '{$accessory->QuantityAvailable}', '{$accessory->CreditCount}', '{$accessory->PeakCreditCount}', '{$accessory->CreditApplicability}');
            {/foreach}
