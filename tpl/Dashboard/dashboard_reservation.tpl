...
<div class="reservation row {$class}" id="{$reservation->ReferenceNumber}">
    ...
    <div class="col-sm-2 col-12">{fullname first=$reservation->FirstName last=$reservation->LastName ignorePrivacy=$reservation->IsUserOwner($UserId)} {if !$reservation->IsUserOwner($UserId)}{html_image src="users.png" altKey=Participant}{/if}</div>
    ...
    {if $checkin}
        <div class="col-sm-1 col-12">
            <button title="{translate key=CheckIn}" type="button" class="btn btn-xs col-12 btn-success btnCheckin" data-referencenumber="{$reservation->ReferenceNumber}" data-url="ajax/reservation_checkin.php?action={ReservationAction::Checkin}">
    ...
    {if $checkout}
        <div class="col-sm-1 col-12">
            <button title="{translate key=CheckOut}" type="button" class="btn btn-xs col-12 btn-success btnCheckin" data-referencenumber="{$reservation->ReferenceNumber}" data-url="ajax/reservation_checkin.php?action={ReservationAction::Checkout}">
