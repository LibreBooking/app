...
phpScheduleIt was designed to make authentication pluggable so that new credential stores can be without changes to the phpScheduleIt source code.  
phpScheduleIt ships with an LDAP authentication plugin.  This should serve as a good example because it is sufficiently complex and covers a typical scenario.
...
This method is only called if the result of Validate() was true.  Typically, this method would synchronize data between the source store and phpScheduleIt, 
although this is not required if the user account data already exists in phpScheduleIt.  
