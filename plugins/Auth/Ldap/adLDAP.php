...
class adLDAP {
	...
	function __construct($options = array())
	{
		...
		if (count($options) > 0)
		{
			...
			if (array_key_exists("host", $options)) { $this->_host = $options["host"]; }
			...
			if (array_key_exists("ldap_version", $options)){ $this->_ldap_version = $options["ldap_version"]; }
			if (array_key_exists("port", $options)){ $this->_port = $options["port"]; }
		}
		...
		if ($this->_use_ssl)
		{
			$this->_conn = ldap_connect("ldaps://".$dc, $this->_port);
		} 
		else 
		{
			$this->_conn = ldap_connect($dc, $this->_port);
		}
		
		//set some ldap options for talking to AD
		ldap_set_option($this->_conn, LDAP_OPT_PROTOCOL_VERSION, $this->_ldap_version);
	...
	function authenticate($username, $password, $prevent_rebind = false)
	{
		...
		{ 
			return false; //prevent null binding
		} 
		...
		if ($this->_ad_username !=NULL && !$prevent_rebind)
		{
			...
			if (!$this->_bind)
			{ 
				throw new Exception("FATAL: AD rebind failed.");
