...
class ReservationRepositoryTests extends TestBase
{
	...
	public function testAddReservationWithOneUserAndOneResource()
	{
		...
		$accessory = new ReservationAccessory(new Accessory(928, "an", 10), 3);
	...
	public function testLoadByIdFullyHydratesReservationSeriesObject()
	{
		...
		$instance1InvitedGuests = array('g1@email.com', 'g2@email.com');
		$instance1ParticipatingGuests = array('g3@email.com', 'g4@email.com');
		$instance2Invitees = array(6);
		$instance2Participants = array(7, 8, 9);
		$instance2ParticipatingGuests = array('g2@email.com', 'g3@email.com');
		$instance2InvitedGuests = array('g4@email.com', 'g5@email.com');
		...
		$accessoryName1 = "1";
		$accessoryName2 = "2";
		$accessoryAvailable1 = 100;
		$accessoryAvailable2 = 200;
		$quantity1 = 11;
		$quantity2 = 22;
		...
		$expected->WithPrimaryResource(new BookableResource($resourceId, $resourceName, $location, $contact, $notes, $minLength, $maxLength, $autoAssign,
															$requiresApproval, $allowMultiDay, $maxParticipants, $minNotice, $maxNotice, null, $scheduleId));
		$expected->WithTitle($title);
		$expected->WithDescription($description);
		$expected->WithResource(new BookableResource($resourceId1, $resourceName, $location, $contact, $notes, $minLength, $maxLength, $autoAssign,
													 $requiresApproval, $allowMultiDay, $maxParticipants, $minNotice, $maxNotice, null, $scheduleId));
		$expected->WithResource(new BookableResource($resourceId2, $resourceName, $location, $contact, $notes, $minLength, $maxLength, $autoAssign,
													 $requiresApproval, $allowMultiDay, $maxParticipants, $minNotice, $maxNotice, null, $scheduleId));
		$expected->WithRepeatOptions($repeatOptions);
		$expected->WithStatus($statusId);
		$accessory1 = new Accessory($accessoryId1, $accessoryName1, $accessoryAvailable1);
		$accessory1->AddResource(1, null, 100);
		$expected->WithAccessory(new ReservationAccessory($accessory1, $quantity1));
		$expected->WithAccessory(new ReservationAccessory(new Accessory($accessoryId2, $accessoryName2, $accessoryAvailable2), $quantity2));
		...
		$reservationResourceRow = new ReservationResourceRow($reservationId, $resourceName, $location, $contact, $notes, $minLength, $maxLength, $autoAssign,
															 $requiresApproval, $allowMultiDay, $maxParticipants, $minNotice, $maxNotice, $scheduleId);
		...
		$reservationAccessoryRow->WithAccessory($accessoryId1, $quantity1, $accessoryName1, $accessoryAvailable1)
								->WithAccessory($accessoryId2, $quantity2, $accessoryName2, $accessoryAvailable2);
		...
		$this->db->SetRow(5,
						  array(array(ColumnNames::RESOURCE_ID => 1, ColumnNames::ACCESSORY_MINIMUM_QUANTITY => null, ColumnNames::ACCESSORY_MAXIMUM_QUANTITY => 100)));    // accessory resources
		$this->db->SetRow(6, array([]));
		$this->db->SetRow(7, $attributeValueRow->Rows());
		$this->db->SetRow(8, $attachmentRow->Rows());
		$this->db->SetRow(9, $reminderRow->Rows());
		$this->db->SetRow(10, $guestRow->Rows());
	...
	public function testWhenCreditsConsumedIncreases_ResourceOnlyPerSlot()
	{
	...
	public function testDoesNotDeductCreditsForPastInstances_ResourceOnlyPerSlot()
	{
		$layout = new FakeScheduleLayout();
		$layout->_SlotCount = new SlotCount(2, 2);
		$userId = 10;
	...
	public function testDoesNotDeductCreditsForWhenNoChangeInCredits_ResourceOnlyPerSlot()
	{
		$layout = new FakeScheduleLayout();
		$layout->_SlotCount = new SlotCount(2, 2);
		$userId = 10;
	...
	public function testChangesGuestsForAllInstances()
	{
		...
		$series = $builder->BuildTestVersion();
		$series->ChangeGuests(array('g3@email.com', 'g4@email.com'), array());
	...
	public function testChangesAccessories()
	{
		$builder = new ExistingReservationSeriesBuilder();
		$series = $builder->BuildTestVersion();
		$accessory1 = new Accessory(1, "1", 1);
		$accessory2 = new Accessory(2, "2", 2);
		$series->WithAccessory(new ReservationAccessory($accessory1, 1));
		$series->ChangeAccessories(array(new ReservationAccessory($accessory2, 2)));
	...
	public function testEventsWhichAreNotNecessaryWhenSeriesIsBranchedAreIgnored()
	{
		...
		$accessory = new ReservationAccessory(new Accessory(9292, "name", 1), 4);
